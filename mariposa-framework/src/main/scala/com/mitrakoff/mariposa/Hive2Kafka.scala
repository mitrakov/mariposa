...
case class Hive2Kafka private (
  ...
  def build(): Runnable = () => {
    ...
    // Converting every row into a JSON for Kafka
    val kafkaPayloadDF = hiveDF.select(to_json(struct(allColumns(hiveDF): _*)).as("value"))

    kafkaPayloadDF.write
      .format("kafka")
  ...
  private def allColumns(df: DataFrame) = df.columns.map(col)
}
